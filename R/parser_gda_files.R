...
read_gda_file <- function(path_to_file, prefix = "") {
  ...
  # create data frames for data
  row_anno <- data.frame(stringsAsFactors = FALSE)
  ms_data <- data.frame(stringsAsFactors = FALSE)
  ...
  # assign data frames
  assign(paste0(prefix, "ms_data"), ms_data, envir = parent.frame())
  assign(paste0(prefix, "col_anno"), col_anno, envir = parent.frame())
  assign(paste0(prefix, "row_anno"), row_anno, envir = parent.frame())
}