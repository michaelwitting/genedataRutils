...
## Creating .ms files (MS1 only)

There is also the possibility to only write MS1 data to files. This is advisable for data were no MS2 has been collected.
...
for(cluster_id in cluster_ids) {
  ...
  raw_files <- unique(mcols(ms1_spectrum_filtered)$RAWFILE)
  
  for(raw_file in raw_files) {
    
    adduct <- "[M+H]+"
    
  file_name <- make.names(paste0(cluster_id, "_", adduct, "_", raw_file, "ms1_only.ms"))
  write_ms_file_ms1only(ms1_spectrum_filtered, precursor, adduct, file_name)
